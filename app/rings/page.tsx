...
export default async function RingsPage({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  try {
    if (process.env.MONGODB_URI) {
      ...
      // Filter for rings category with optional search
      const filter = {
        category: { $regex: /ring/i },
        ...(q ? {
          $or: [
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="lux-heading text-2xl sm:text-3xl font-semibold tracking-tight heading-underline">Rings</h1>
          <p className="text-muted">{q ? `Showing ring results for "${q}"` : "Explore our stunning collection of gemstone rings."}</p>
        </div>
      </div>
...
export async function generateMetadata({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }): Promise<Metadata> {
  ...
  const baseTitle = "Rings";
  const title = q ? `${baseTitle} – ${q}` : baseTitle;
  const canonical = q ? `${site}/rings?q=${encodeURIComponent(q)}` : `${site}/rings`;
  return {
    title,
    description: q ? `Search results for "${q}" in rings.` : "Browse our stunning collection of gemstone rings.",
    alternates: { canonical },
    openGraph: { title, description: q ? `Search results for "${q}" in rings.` : "Browse our stunning collection of gemstone rings.", url: canonical },
