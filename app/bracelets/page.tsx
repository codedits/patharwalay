...
export default async function BraceletsPage({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  try {
    if (process.env.MONGODB_URI) {
      ...
      // Filter for bracelets category with optional search
      const filter = {
        category: { $regex: /bracelet/i },
        ...(q ? {
          $or: [
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="lux-heading text-2xl sm:text-3xl font-semibold tracking-tight heading-underline">Bracelets</h1>
          <p className="text-muted">{q ? `Showing bracelet results for "${q}"` : "Discover our elegant collection of gemstone bracelets."}</p>
        </div>
      </div>
...
export async function generateMetadata({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }): Promise<Metadata> {
  ...
  const baseTitle = "Bracelets";
  const title = q ? `${baseTitle} – ${q}` : baseTitle;
  const canonical = q ? `${site}/bracelets?q=${encodeURIComponent(q)}` : `${site}/bracelets`;
  return {
    title,
    description: q ? `Search results for "${q}" in bracelets.` : "Browse our elegant collection of gemstone bracelets.",
    alternates: { canonical },
    openGraph: { title, description: q ? `Search results for "${q}" in bracelets.` : "Browse our elegant collection of gemstone bracelets.", url: canonical },
