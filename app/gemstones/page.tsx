...
export default async function GemstonesPage({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  try {
    if (process.env.MONGODB_URI) {
      ...
      // Filter for gemstones category OR uncategorized products, with optional search
      const categoryMatcher = { $or: [ { category: { $regex: /gemstone/i } }, { category: { $exists: false } }, { category: "" } ] };
      const filter = q
        ? {
            $and: [
              categoryMatcher,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="lux-heading text-2xl sm:text-3xl font-semibold tracking-tight heading-underline">Gemstones</h1>
          <p className="text-muted">{q ? `Showing gemstone results for "${q}"` : "Discover our exquisite collection of loose gemstones."}</p>
        </div>
      </div>
...
export async function generateMetadata({ searchParams }: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }): Promise<Metadata> {
  ...
  const baseTitle = "Gemstones";
  const title = q ? `${baseTitle} – ${q}` : baseTitle;
  const canonical = q ? `${site}/gemstones?q=${encodeURIComponent(q)}` : `${site}/gemstones`;
  return {
    title,
    description: q ? `Search results for "${q}" in gemstones.` : "Browse our exquisite collection of loose gemstones.",
    alternates: { canonical },
    openGraph: { title, description: q ? `Search results for "${q}" in gemstones.` : "Browse our exquisite collection of loose gemstones.", url: canonical },
